...
static dispmode *display;
static camcontrol *controller;
...
{
	display->begin_eye(viewportID);
}

void viewmat_end_eye(int viewportID)
{
	display->end_eye(viewportID);
}
...
static void viewmat_validate_ipd(const float viewmatrix[16], int viewportID)
{
	...
	if(viewportID == 1)
	{
		...
		if(display->eye_type(0) == VIEWMAT_EYE_RIGHT)
			flip = -1;
...
/** Get the camera's current position.
    ...
    @param pos A 3 component vector where the position should be written into.

    @param eye Common values include VIEWMAT_EYE_MIDDLE,
    VIEWMAT_EYE_LEFT, VIEWMAT_EYE_RIGHT.
*/
void viewmat_get_pos(float pos[3], viewmat_eye eye)
{
	float viewmatrix[16];
	controller->get(viewmatrix, eye);

	/* We can't just get the camera's position directly from
	 * the view matrix because it has translation and rotation
	 * components multiplied together. For info on how to
	 * retrieve the information, see:
	 * https://gamedev.stackexchange.com/q/22283
	 */
	mat4f_invert(viewmatrix);
	float temppos[4];
	mat4f_getColumn(temppos, viewmatrix, 3); // 3 = get 4th column.
	vec3f_copy(pos, temppos);
}
 ...
 * this function. NOTE: Use viewmat_get_pos() to get the position of
 * the camera. It allows you to request the point between the two eyes
 * using viewportID = -1. This function doesn't work for that because
 * it requires that each viewportID actually correspond to an OpenGL
 * viewport.
 *
...
viewmat_eye viewmat_get(float viewmatrix[16], float projmatrix[16], int viewportID)
{
	...
	if(viewportID == -1)
		...
		eye = display->eye_type(viewportID);
	
	int viewport[4]; // x,y of lower left corner, width, height
	display->get_viewport(viewport, viewportID);

	/* Get the current projection matrix. */
	display->get_projmatrix(projmatrix, viewportID);
	...
	{
		if(display->provides_projmat_only())
		{
		...
		else
		{
			...
			float frustum[6];
			display->get_frustum(frustum, viewportID);
...
{
	display->get_viewport(viewportValue, viewportNum);
}
...
{
	return display->get_framebuffer(viewportID);
}
