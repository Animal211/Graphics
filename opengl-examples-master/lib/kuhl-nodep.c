...
char* kuhl_find_file(const char *filename)
{
	...
	strncpy(commonDirs[commonDirsLen++], "../config", 255);
#else
	strncpy(commonDirs[commonDirsLen++], "../samples", 255); // Find fragment programs in samples directory
	strncpy(commonDirs[commonDirsLen++], "../models", 255);
	strncpy(commonDirs[commonDirsLen++], "../", 255);
	...
	for(int i=0; i<origLen; i++)
		if(commonDirs[i][0] != '/')
		{
			int retVal = snprintf(commonDirs[commonDirsLen++], 256, "%s/%s", dir, commonDirs[i]);
			if(retVal >= 256)
				msg(MSG_ERROR, "A directory we have in the search path is too long for kuhl_find_file(): %s/%s", dir, commonDirs[i]);
		}
#endif
