...
int kuhl_errorcheckFileLine(const char *file, int line, const char *func)
{
	...
	if(errCode != GL_NO_ERROR)
	{
		msg_details(MSG_ERROR, file, line, func,
		            "OpenGL error '%s' (0x%x) occurred before %s:%d",  gluErrorString(errCode), errCode, file, line);
		return 1;
	}
	return 0;
}
...
{
	/* Call srand48() to ensure that random numbers are random each
	...
	// Use linear RGB in GLSL if we can. This setting will make it so
	// GLSL's texture() function will return linear color (because we
	// use GL_SRGB8_ALPHA8 or GL_SRGB8 while loading textures) and
	// will convert the color back to sRGB (because we use
	// glEnable(GL_FRAMEBUFFER_SRGB) ).
	if(kuhl_config_int("color.linear", 1, 1) == 1)
	{
		msg(MSG_DEBUG, "Using linear color in fragment program. Set color.linear=0 to disable.");
		...
		msg(MSG_DEBUG, "Using non-linear sRGB color in fragment program. Set color.linear=1 to convert to linear colorspace.");
...
GLint kuhl_get_uniform(const char *uniformName)
{
	...
	{
		msg(MSG_ERROR, "Uniform variable '%s' is missing or inactive in GLSL program %d.\n", uniformName, currentProgram);
		missingUniformCount++;
...
void kuhl_geometry_indices(kuhl_geometry *geom, GLuint *indices, GLuint indexCount)
{
	...
	{
		//UNCOMMENT!!!!!!!!!!!!//msg(MSG_WARNING, "indexCount was zero or indices array was NULL\n");
		return;
	}
	...
	/* TODO: Check if indices are already set? If so, print an error
	 * message and/or free the old indices buffer before making a new
	 * one to replace it. */
...
void kuhl_geometry_draw(kuhl_geometry *geom)
{
	...
	{
	//temp commented out so I can read prints	msg(MSG_WARNING, "You tried to draw geometry which contained 0 vertices.");
		return;
	}
	if(geom->attrib_count == 0)
	{
		msg(MSG_WARNING, "You tried to draw geometry which had no attributes. It needs at least one attribute (i.e., vertex position)");
		return;
	}
...
{
	// We delete each kuhl_geometry object in the list, but we
...
GLuint kuhl_read_texture_array(const unsigned char* array, int width, int height, int components, GLuint wrapS, GLuint wrapT)
{
	...
	GLuint internalformat = kuhl_config_int("color.linear", 1, 1) ? GL_SRGB8 : GL_RGB8;
	GLuint imageformat = GL_RGB;
	GLuint pixeldatatype = GL_UNSIGNED_BYTE;
	if(components == 4)
	{
		internalformat = kuhl_config_int("color.linear", 1, 1) ? GL_SRGB8_ALPHA8 : GL_RGBA8;
		imageformat = GL_RGBA;
	}
...
{
#ifdef KUHL_UTIL_USE_IMAGEMAGICK
	static const char *exten = "tif";
#else
	static const char *exten = "bmp";
#endif

	static int kuhl_video_record_frame = 0; // filename counter
	static long time_of_next_picture = 0;    // time to take next picture
	int usec_between_pictures = 1000000/fps;  // integer division...

	if(time_of_next_picture == 0) // if first time called.
	{
		...
		time_of_next_picture = kuhl_microseconds() + usec_between_pictures;
	}


	if(kuhl_microseconds() > time_of_next_picture)
	{
		time_of_next_picture += usec_between_pictures;
...
static int kuhl_print_aiNode_info(const char *modelFilename, const struct aiNode *node)
{
	...
	{
		char tmp[2048];
		int retVal = snprintf(tmp, 2048, "%s->%s", parent->mName.data, name);
		if(retVal >= 2048)
			msg(MSG_WARNING, "String describing node was truncated in kuhl_print_aiNode_info()");
		strcpy(name, tmp);
