...
void display()
{
	...
	for(int viewportID=0; viewportID<viewmat_num_viewports(); viewportID++)
	{
		...
		mat4f_scale_new(scaleMatrix, 1, 1, 1);
		...
		glUniform1f(kuhl_get_uniform("time"), glfwGetTime());
...
void init_geometryFire(kuhl_geometry* geom, GLuint prog) {
	int particalcount = 10000;
	kuhl_geometry_new(geom, prog, particalcount, GL_POINTS);
	GLfloat* vertexData;
	vertexData = (GLfloat*)malloc(3* particalcount * sizeof(GLfloat));
	int vertIndex = 0;

	for (int i = 0; i < particalcount; i++) {
		vertexData[vertIndex++] = drand48();
		vertexData[vertIndex++] = 0;
		vertexData[vertIndex++] = drand48();
	}
	...
	GLuint texId1 = 0;
	GLuint texId2 = 0;
	float aspectRatio1 = kuhl_read_texture_file("../images/fire-particle.png", &texId1);
	float aspectRatio2 = kuhl_read_texture_file("../images/firesmokeparticlenew.png", &texId2);
	msg(MSG_DEBUG, "Aspect ratio of image is %f\n", aspectRatio1); // write message to log.txt
...
int main(int argc, char** argv)
{
	...
	init_geometryFire(&fire, program);
	...
	float initCamPos[3]  = {0,2,6}; // location of camera
	float initCamLook[3] = {6,0,6}; // a point the camera is facing at
