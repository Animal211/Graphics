===
=== Compiling code for the first time
===

This code depends on several libraries:

- OpenGL 3.2 (this is provided by most OS's by default)
- GLFW: Creates the window, handles mouse/keyboard input.
- GLEW: Helps identify capabilities of graphics card.

There are two other libraries which you very likely want:
- Assimp: Allows you to load 3D models created by blender, etc.
- ImageMagick: Allows code to load more image formats (otherwise, only
  JPEG and PNG is supported)

Each of these libraries need to be installed and cmake needs to be
able to find those libraries. This folder also contains files with
additional information about other libraries that are available.

So, the general installation process is:
1) Install the libraries.
2) Make sure that cmake can find those libraries.
3) Compile the code.
4) Try running the "triangle" program in the "bin" folder.


The exact way that you install the libraries depends on the
platform. Each library also has a separate file outlining additional
information (for example, if you wish to compile the program from
scratch).

Windows: Follow instructions in WINDOWS-VisualStudio.txt.

Linux: Follow instructions in Linux.txt

MacOS: Follow instructions in Linux.txt. However, you will need to
also look at the txt files for the individual libraries to learn how
to install them.
